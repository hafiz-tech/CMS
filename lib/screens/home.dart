...
class _HomeState extends State<Home> with TickerProviderStateMixin {
  ...
  Widget buildHomeBannerOne(context, HomePresenter homeData) {
    if (homeData.isBannerInitial &&
        homeData.bannerImageList.length == 0) {
      return Padding(
          padding:
              const EdgeInsets.only(left: 18.0, right: 18, top: 10, bottom: 20),
          child: ShimmerHelper().buildBasicShimmer(height: 120));
    } else if (homeData.bannerImageList.length > 0) {
      return Padding(
        ...
        child: CarouselSlider(
          ...
          items: homeData.bannerImageList.map((i) {
            return Builder(
              builder: (BuildContext context) {
                return Padding(
                  padding: const EdgeInsets.only(
    ...
    } else if (!homeData.isBannerInitial &&
        homeData.bannerImageList.length == 0) {
      return Container(
          height: 100,
          child: Center(
